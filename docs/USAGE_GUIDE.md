...
A session shutdown or switch interrupts the monitor: its in-memory process and wait are discarded without a terminal workflow wake. Inspect the resumed workflow and start a new monitor if the work still needs to run. On a session switch, memory-scoped workflow controllers are retained for linked-task ownership; memory state is lost after process exit.
